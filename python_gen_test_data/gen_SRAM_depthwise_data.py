...
BIAS_BASE_WORD   = 0x0000_2000
IPSUM_BASE_WORD  = 0x0000_2500
...
ifmap = np.random.randint(0, 6, size=(C, EFFECTIVE_H, EFFECTIVE_W), dtype=np.uint8)
ipsum = np.random.randint(5, 11, size=(C, EFFECTIVE_H, EFFECTIVE_W), dtype=np.uint16)
...
# === 計算 Golden Opsum（考慮 padding） ===
is_bias = False
golden_opsum = np.zeros((C, EFFECTIVE_H, EFFECTIVE_W), dtype=np.int32)
for c in range(C):
    for r in range(EFFECTIVE_H):
        for col in range(EFFECTIVE_W):
            psum = int(bias[c]) if is_bias else int(ipsum[c][r][col])
...
for c in range(2):  # 前 2 個通道
    for r in range(1):  # 前 2 個 row
        for col in range(3):  # 前 3 個 column
            print("=" * 60)
